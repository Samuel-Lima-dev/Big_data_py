...
# Matriz de correlação (Pearson) e heatmap
corr_matrix = df_corr.drop('Regiao', axis=1).corr(method='pearson')
plt.figure(figsize=(10, 7))
sns.heatmap(corr_matrix, annot=True, cmap='YlOrBr', fmt=".2f")
plt.title('Matriz de Correlação (Pearson) - Seca Excepcional/Extrema/Severa x Focos por Bioma (por Região)')
plt.tight_layout()
plt.savefig('graficos/Seca Excepcional/Extrema/Severa x Focos por Bioma.png')
plt.close()